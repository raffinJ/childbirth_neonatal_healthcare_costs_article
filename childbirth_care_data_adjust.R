...
dados_sih <- read_csv("dados_sih.csv")


# 2) Selecting the variables of interest, according to their name in the database
dados_sih <- dados_sih |> select(c("ANO_CMPT", "DT_INTER", "DT_SAIDA", "MARCA_UTI", "PROC_REA", "VAL_SH", "VAL_SP", "VAL_TOT", "VAL_UTI", "US_TOT", "DIAG_PRINC", "MUNIC_MOV", "IDADE", "COD_IDADE", "DIAS_PERM","UTI_MES_TO", "QT_DIARIAS", "MORTE"))


# 3) Adjusting the database for future analyses

# Including the SIGTAP table of SUS procedures. This table decodes procedure codes into their respective names
dados_sih_neo$PROC_REA <- as.factor(dados_sih_neo$PROC_REA) 
TB_SIGTAP <- read.dbc("TB_SIGTAP.dbc") # Download the SIGTAP table from a local file
dados_sih_neo <- dados_sih_neo %>%
  left_join(TB_SIGTAP, c("PROC_REA"="CHAVE")) # Adds a new column to the df called “DS_REGRA” with the names of the procedures 


# Adding a column indicating whether or not that inpatient record was admitted to an ICU
dados_sih$USO_UTI=ifelse(dados_sih$MARCA_UTI %in% c("00"), c("No"), "Yes")


# Modifying the "death" (in Portuguese - "morte") column indicating whether or not that inpatient record led to death
dados_sih$MORTE=ifelse(dados_sih$MORTE %in% c("1"), c("Death"), "No")


# Adding column to group delivery type (in Portuguese - via de parto) procedures into just 2 categories: vaginal delivery and caesarean delivery
dados_sih$VIA_PARTO = ifelse(dados_sih$DS_REGRA %in% c("PARTO CESARIANO", "PARTO CESARIANO C/ LAQUEADURA TUBARIA", "PARTO CESARIANO EM GESTACAO DE ALTO RISCO"),c("caesarean"),
                      ifelse(dados_sih$DS_REGRA %in% c("PARTO NORMAL", "PARTO NORMAL EM CENTRO DE PARTO NORMAL (CPN)", "PARTO NORMAL EM GESTACAO DE ALTO RISCO"),c("vaginal birth"), "Ignored"))
